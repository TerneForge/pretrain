...
class PhiAttention(nn.Module):
    ...
    def __init__(self, config: PhiConfig, layer_idx: int):
        ...
        self.is_causal = True
        self.q_proj = QLinear(config.hidden_size, config.num_attention_heads * self.head_dim, bias=True)
        self.k_proj = QLinear(config.hidden_size, config.num_key_value_heads * self.head_dim, bias=True)
        self.v_proj = QLinear(config.hidden_size, config.num_key_value_heads * self.head_dim, bias=True)
        self.dense = QLinear(config.num_attention_heads * self.head_dim, config.hidden_size, bias=True)
...
class QPhiForCausalLM(PhiPreTrainedModel, GenerationMixin):
