...
class MinimalTrainer(Trainer):
    ...
    def evaluate_hellaswag(
        ...
        with torch.no_grad():
            for step, inputs in enumerate(eval_dataloader):
                ...
                mask = self.accelerator.pad_across_processes(mask, dim=1, pad_index=0)
                ...
                mask = mask.reshape(psbs, 4, logits.shape[-2])
                ...
                mask = mask.reshape(mask.shape[0]*4, mask.shape[-1])
    ...
    def _inner_training_loop(
        ...
        # copied from transformers==4.44.0
        # All modified code is marked with `MODIFIED BY YULANMINI`
